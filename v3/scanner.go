...
func (t Token) Sep() string {
	...
	return string(s.buf[s.toks[t.index].sepOff:s.toks[t.index].off])
}
...
func (t Token) Src() string {
	...
	if t.index < int32(len(s.toks))-1 {
		next = s.toks[t.index+1].off
	}
	return string(s.buf[s.toks[t.index].off:next])
}
...
func (t *tok) position(s *source) (r token.Position) {
	return token.Position(s.file.PositionFor(mtoken.Pos(s.base+t.off), true))
}
...
out:
	...
	if s.tok.ch = int32(keywords[string(s.buf[s.tok.off:s.off])]); s.tok.ch == 0 {
